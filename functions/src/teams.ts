...
export const enviarConviteTime = functions.https.onCall(async (data: { teamId: string; userId: string }, context) => {
  ...
  try {
    ...
    logger.business('Convite de time enviado', { teamId, userId }, contextData);
    return { success: true };

  } catch (error: any) {
    logger.error('Erro ao enviar convite de time', { error: error.message }, contextData);
    throw error;
  }
});
...
export const responderConviteTime = functions.https.onCall(async (data: { inviteId: string; resposta: 'accept' | 'reject' }, context) => {
  ...
  try {
    ...
    logger.business('Convite de time respondido', { inviteId, resposta }, contextData);
    return { success: true };

  } catch (error: any) {
    logger.error('Erro ao responder convite de time', { error: error.message }, contextData);
    throw error;
  }
});
...
export const listarConvitesUsuario = functions.https.onCall(async (data: {}, context) => {
  ...
  try {
    ...
    logger.error('Erro ao listar convites do usuário', { error: error.message }, contextData);
    throw error;
  }
});
...
export const cancelarConviteTime = functions.https.onCall(async (data: { inviteId: string }, context) => {
  ...
  try {
    ...
    logger.business('Convite de time cancelado', { inviteId }, contextData);
    return { success: true };

  } catch (error: any) {
    logger.error('Erro ao cancelar convite de time', { error: error.message }, contextData);
    throw error;
  }
}); 